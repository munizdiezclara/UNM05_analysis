...
```{r, echo = FALSE}
test1 <- test1 %>%
  mutate(similarity = case_when(session == 1 ~ "No subtle", 
                                session == 2 ~"Subtle", 
                                session == 3 ~ "Very subtle"))
#plot test1 accuracy
m_acc_test1 <- test1 %>%
  group_by(cue_type, similarity) %>%
  summarise(mean_acc = mean(acc, na.rm = TRUE), 
            sd_acc = sd(acc, na.rm = TRUE)/sqrt(length(acc)))
ggplot(data = m_acc_test1) +
  geom_col(mapping = aes(x = cue_type, y = mean_acc, fill = similarity)) +
...
ggplot(data = m_mem_test1) +
  geom_col(mapping = aes(x = cue_type, y = mean_mem_score, fill = similarity)) +
...
When only hits are taken into account in test 1, memory score is corrected in test 1, there is a significant effect of predictiveness (`r apa(c_ANOVA_mem_score_test1, effect = "predictiveness")`), but the bayesian evidence is anecdotal (`r report_BF_and_error(c_bay_ANOVA_mem_score_test1[2])`. However, there were no significant effects of the type of test (`r apa(c_ANOVA_mem_score_test1, effect = "session")`, `r report_BF_and_error(c_bay_ANOVA_mem_score_test1[1])`) nor the interaction (`r apa(c_ANOVA_mem_score_test1, effect = "session:predictiveness")`, `r report_BF_and_error(c_bay_ANOVA_mem_score_test1_interaction[1])`).
